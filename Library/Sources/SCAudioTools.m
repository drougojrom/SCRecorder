...
+ (void)mixAudio:(AVAsset*)audioAsset startTime:(CMTime)startTime withVideo:(NSURL*)inputUrl affineTransform:(CGAffineTransform)affineTransform  toUrl:(NSURL*)outputUrl outputFileType:(NSString*)outputFileType withMaxDuration:(CMTime)maxDuration withCompletionBlock:(void(^)(NSError *))completionBlock {
    ...
    AVURLAsset * fileAsset = [AVURLAsset URLAssetWithURL:inputUrl options:@{AVURLAssetPreferPreciseDurationAndTimingKey: @YES}];
    
    NSArray * videoTracks = [fileAsset tracksWithMediaType:AVMediaTypeVideo];
    
    CMTime duration = ((AVAssetTrack*)videoTracks[0]).timeRange.duration;
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^ {
        ...
        if (exportSession.error != nil) {
            ...
            NSString * subLocalizedDescription = userInfo[NSLocalizedDescriptionKey];
            [userInfo removeObjectForKey:NSLocalizedDescriptionKey];
            userInfo[NSLocalizedDescriptionKey] = @"Failed to mix audio and video";
            userInfo[@"OutputFileType"] = exportSession.outputFileType;
            userInfo[@"OutputUrl"] = exportSession.outputURL;
            userInfo[@"CauseLocalizedDescription"] = subLocalizedDescription;
            
            userInfo[@"AllExportSessions"] = [AVAssetExportSession allExportPresets];
